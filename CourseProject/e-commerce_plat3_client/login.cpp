...
Login::Login(MainWindow *w, QVector<User *> &ulist, QJsonObject &pjson, QJsonObject &cjson, QJsonObject &ojson, QWidget *parent)
    : QWidget(parent), user_list(ulist), product_json(pjson),cart_json(cjson), order_json(ojson)
{
    now_window = w;
...
void Login::logIn()
{
    ...
    for (auto i = user_list.begin(); i < user_list.end(); i++)
        if ((*i)->getUserName() == username_trying && (*i)->getUserType() == usertype_trying)
        {
            ...
            {
                error_text->hide();
                now_window = new MainWindow(*i, user_list, product_json, cart_json, order_json);
